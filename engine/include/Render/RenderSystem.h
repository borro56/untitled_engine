...
private:
    ...
    void updateObjects(int newAmount);
...
{
    auto app = reinterpret_cast<RenderSystem*>(glfwGetWindowUserPointer(window));
...
void RenderSystem::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
...
QueueFamilyIndices RenderSystem::findQueueFamilies(VkPhysicalDevice device)
{
...
SwapChainSupportDetails RenderSystem::querySwapChainSupport(VkPhysicalDevice device)
{
...
void RenderSystem::cleanupSwapChain()
{
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        for (int j = 0; j < maxAmountOfObjects; ++j)
        {
...
void RenderSystem::createUniformBuffers()
{
    ...
    uniformBuffers.resize(maxAmountOfObjects);
    uniformBuffersMemory.resize(maxAmountOfObjects);

    for (int j = previousAmount; j < maxAmountOfObjects; ++j)
    {
        uniformBuffers[j].resize(swapChainImages.size());
        uniformBuffersMemory[j].resize(swapChainImages.size());

        for (size_t i = 0; i < swapChainImages.size(); i++) {
            createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffers[j][i], uniformBuffersMemory[j][i]);
        }
    }
}

void RenderSystem::updateObjects(int newAmount)
{
...
void RenderSystem::createDescriptorPool()
{
    ...
    poolSize.descriptorCount = maxAmountOfObjects > 0 ? static_cast<uint32_t>(swapChainImages.size() * maxAmountOfObjects) : 1;

    VkDescriptorPoolCreateInfo poolInfo{};
    poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    poolInfo.poolSizeCount = 1;
    poolInfo.pPoolSizes = &poolSize;
    poolInfo.maxSets = maxAmountOfObjects > 0 ? static_cast<uint32_t>(swapChainImages.size() * maxAmountOfObjects) : 1;
...
void RenderSystem::createDescriptorSets()
{
    ...
    descriptorSets.resize(maxAmountOfObjects);

    for (int k = 0; k < maxAmountOfObjects; ++k)
    {
        descriptorSets[k].resize(swapChainImages.size());
        if (vkAllocateDescriptorSets(device, &allocInfo, descriptorSets[k].data()) != VK_SUCCESS) {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        for (int j = 0; j < maxAmountOfObjects; ++j)
        {
            ...
            descriptorWrite.dstSet = descriptorSets[j][i];
